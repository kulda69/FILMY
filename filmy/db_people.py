...
def lookup_person_by_query(query: str, candidates_limit: int=5) -> dict[str, Any] | None:
    ...
    ordered_candidates = sorted(candidates, key=lambda item: (0 if item['nconst'] == selected_key else 1, -(item.get('birth_year') or 0), item['primary_name']))
    result = {'query': query, 'selected_nconst': selected_key, 'selected': db._build_person_lookup_candidate(selected, query=query, is_selected=True), 'candidates': [db._build_person_lookup_candidate(candidate, query=query, is_selected=candidate['nconst'] == selected_key) for candidate in ordered_candidates[:max(candidates_limit, 1)]], 'candidate_count': len(candidates)}
...
def render_person_presentation(presentation: dict[str, Any]) -> str:
    ...
    sections = [('Directed', filmography.get('directed') or []), ('Created by', filmography.get('created') or []), ('Written', filmography.get('written') or []), ('Acted in', filmography.get('acted') or [])]
