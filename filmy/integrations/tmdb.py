...
_TMDB_CONFIGURATION_CACHE: dict[str, Any] | None = None
...
def sync_title_from_imdb(tconst: str, locale: str = PRIMARY_LOCALE) -> dict[str, Any]:
    _require_token()
    match = _api_get(f"/find/{tconst}", {"external_source": "imdb_id", "language": locale})
    ...
    if movie_results:
        ...
        raise TmdbNotFoundError(f"TMDB nenašlo titul pro IMDb ID {tconst}.")

    tmdb_id = int(first["id"])
    detail = _api_get(f"/{media_type}/{tmdb_id}", {"language": locale})
    providers = _api_get(f"/{media_type}/{tmdb_id}/watch/providers")
...
def fetch_assets_for_title(tconst: str, fetch_reason: str) -> dict[str, Any]:
    ...
    if mapping is None:
        raise TmdbApiError("Titul ještě nemá TMDB mapování. Nejprve spusť sync.")
    if mapping.get("sync_status") == "not_found" or int(mapping.get("tmdb_id") or 0) <= 0:
        raise TmdbApiError("Titul je v TMDB označený jako nenalezený, assety nelze stáhnout.")

    detail = get_content_detail(tconst)
    if detail is None:
        raise TmdbApiError("Titul v katalogu neexistuje.")

    tmdb = detail.get("tmdb") or {}
    details = tmdb.get("details") or {}
    configuration = _get_tmdb_configuration()
...
def fetch_person_portrait(nconst: str, fetch_reason: str) -> dict[str, Any]:
    ...
    detail = _api_get(f"/person/{tmdb_person_id}")
    ...
    content = _binary_get(f"{base_url}{profile_size}{profile_path}")
    local_path.write_bytes(content)
...
def enrich_library_from_tmdb(
    ...
    for target in targets:
        ...
        try:
            status_before = get_tmdb_status(tconst)
            ...
            for locale in _missing_locales(status_before["locales"]):
                ...
                sync_results.append(sync_title_from_imdb(tconst, locale=locale))

            current_stage = "asset_check"
            status_after_sync = get_tmdb_status(tconst)
            ...
            if _missing_asset_kinds(tmdb_detail_after_sync, status_after_sync["assets"]):
                ...
                assets_result = fetch_assets_for_title(tconst, fetch_reason=_fetch_reason_for_priority(target["priority"]))
            ...
            target_result["tmdb_status"] = get_tmdb_status(tconst)
            target_result["elapsed_seconds"] = round(time.monotonic() - started_at, 3)
            emit(
                {
                    "phase": "item",
            ...
            summary["not_found"] += 1
            _mark_tmdb_not_found(tconst, str(exc))
            ...
            summary["errors"] += 1
            _try_mark_existing_mapping_error(tconst, str(exc))
            ...
            summary["errors"] += 1
            _try_mark_existing_mapping_error(tconst, f"{type(exc).__name__}: {exc}")
            target_result["status"] = "error"
...
def _api_get(path: str, query: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    body = _request_with_retry(request, timeout=30, error_prefix="TMDB request")
    return json.loads(body.decode("utf-8"))


def _binary_get(url: str) -> bytes:
    request = Request(url, headers={"Accept": "*/*"})
    return _request_with_retry(request, timeout=60, error_prefix="Stažení assetu")


def _get_tmdb_configuration() -> dict[str, Any]:
    global _TMDB_CONFIGURATION_CACHE
    if _TMDB_CONFIGURATION_CACHE is None:
        _TMDB_CONFIGURATION_CACHE = _api_get("/configuration")
    return _TMDB_CONFIGURATION_CACHE
...
def _try_mark_existing_mapping_error(tconst: str, message: str) -> None:
    try:
        ...
        # Nejhorší varianta je přijít o doplňkový status update, ne shodit celý batch.
        return
...
def _require_token() -> str:
    ...
    if not token:
        raise TmdbConfigError("Chybí TMDB_API_READ_ACCESS_TOKEN v prostředí.")
    return token
    ...
    for attempt in range(1, TMDB_MAX_RETRIES + 1):
        ...
        try:
            ...
            if exc.code == 429 and attempt < TMDB_MAX_RETRIES:
                retry_after = _retry_delay_seconds(exc, attempt)
...
def _wait_for_rate_limit_slot() -> None:
    ...
    if len(_REQUEST_TIMESTAMPS) >= TMDB_SAFE_RATE_LIMIT_PER_SECOND:
        sleep_for = max(0.0, 1.0 - (now - _REQUEST_TIMESTAMPS[0]))
        if sleep_for > 0:
