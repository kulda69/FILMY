...
from filmy.background_jobs import read_background_activity_signal
...
def _read_new_activity_signal(previous_marker: int | None) -> tuple[dict[str, object] | None, int | None]:
    ...
    return read_background_activity_signal(), current_marker
...
def main() -> int:
    ...
    initial_person_portrait_pending = len(get_person_portrait_targets(limit=args.person_portrait_batch_size))
    initial_person_detail_pending = (
        len(get_person_detail_cache_targets(limit=args.person_detail_batch_size))
        if args.person_detail_batch_size > 0
        else 0
    )
    _emit(
        {
    ...
    try:
        while True:
            ...
            person_portrait_pending_before = len(get_person_portrait_targets(limit=args.person_portrait_batch_size))
            person_detail_pending_before = (
                len(get_person_detail_cache_targets(limit=args.person_detail_batch_size))
                if args.person_detail_batch_size > 0
                else 0
            )
            ...
            priority_signal, signal_marker = _read_new_activity_signal(signal_marker)
            priority_tconsts = []
            if priority_signal is not None:
                target_tconst = str(priority_signal.get("target_tconst") or "").strip()
                if target_tconst:
                    priority_tconsts.append(target_tconst)

            progress_state["stage"] = "tmdb"
            run_tmdb_pass = args.tmdb_batch_size > 0 and (
                tmdb_remaining_before > 0 or bool(priority_tconsts)
            )
            tmdb_result = enrich_library_from_tmdb(
                limit=args.tmdb_batch_size,
                priority_tconsts=priority_tconsts,
            ) if run_tmdb_pass else {
                "processed": 0,
                "skipped": 0,
                "synced": 0,
                "not_found": 0,
            ...
            person_portrait_pending_after = len(get_person_portrait_targets(limit=args.person_portrait_batch_size))
            ...
            if args.person_detail_batch_size > 0:
                ...
                if person_detail_result["written"] or person_detail_result["cache_hits"] or person_detail_result["missing_detail"]:
                    person_detail_pending_after = 0
                else:
                    person_detail_pending_after = person_detail_pending_before
            else:
                person_detail_result = {
                    "written": 0,
                    "cache_hits": 0,
                    "missing_detail": 0,
                }
            ...
            tmdb_meaningful_work = any(
                [
                    tmdb_result["synced"],
                    tmdb_result["partials"],
                    tmdb_result["not_found"],
